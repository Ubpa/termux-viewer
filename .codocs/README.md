...
│   └── commit-msg
└── docs/               ← 源码树的语义镜像
    ├── src.md          ← src/ 目录的架构文档（含索引表、架构说明、设计决策等）
    └── src/
...
克隆后执行（通过 more-hooks 注册，不直接写 `.git/hooks/`）：

```bash
python .codocs/scripts/setup-hooks.py
```

若 `.git/hooks/` 下已有其他工具直装的 hook，`--force` 会自动 rename 为 `.bak` 并由 dispatcher 接管：

```bash
python .more-hooks/more-hooks.py register . \
    --hook pre-commit --id codocs \
    --script .codocs/hooks/pre-commit --priority 50 --symlink --force
```
...
- ✅ 父目录 MD：无新增/删除/改名文件，且架构说明、设计决策等其他内容也未受影响
...
- .codocs/docs/include/UCommon.md: 无新增/删除/改名文件，架构说明未受影响
```
...
`[STALE_DIR]` 表示子 MD 有更新，父目录文档（含架构说明、设计决策等，不只是索引表）可能需同步。
