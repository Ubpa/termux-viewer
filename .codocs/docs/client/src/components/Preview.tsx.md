...
**渲染分支**：image 直接 `<img src="/api/read?...">` 不发 fetch；markdown 走 `react-markdown + remark-gfm + remark-frontmatter`（GFM + YAML front matter 支持，class `markdown-body`）；code 走 `highlight.js` + 行号注入（字符串拼接 `<span class="line-num">`）；text 用 `<pre>` 纯文本；binary 显示占位。
